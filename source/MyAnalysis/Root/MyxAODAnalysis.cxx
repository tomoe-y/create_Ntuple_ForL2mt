...
StatusCode MyxAODAnalysis :: initialize ()
{
  ...
  m_nMdtHits = std::make_unique<std::vector<uint32_t>>();
  m_mdtHitOnlineId = std::make_unique<std::vector<std::vector<uint32_t>>>();
  m_mdtHitOfflineId = std::make_unique<std::vector<std::vector<int>>>();
  m_mdtHitIsOutlier = std::make_unique<std::vector<std::vector<int>>>();
  m_mdtHitChamber = std::make_unique<std::vector<std::vector<int>>>();
  m_mdtHitR = std::make_unique<std::vector<std::vector<float>>>();
  m_mdtHitZ = std::make_unique<std::vector<std::vector<float>>>();
  m_mdtHitPhi = std::make_unique<std::vector<std::vector<float>>>();
  m_mdtHitresidual = std::make_unique<std::vector<std::vector<float>>>();
  m_mdtHitTime = std::make_unique<std::vector<std::vector<float>>>();
  m_mdrHitSpace = std::make_unique<std::vector<std::vector<float>>>();
  m_mdtHitSigma = std::make_unique<std::vector<std::vector<float>>>();
 ...
 //l2sa
  ...
  //mdt
  mytree->Branch ("nMdtHits", &*m_nMdtHits);
  mytree->Branch ("mdtHitOnlineId", &*m_mdtHitOnlineId);
  mytree->Branch ("mdtHitOfflineId", &*m_mdtHitOfflineId);
  mytree->Branch ("mdtHitIsOutlier", &*m_mdtHitIsOutlier);
  mytree->Branch ("mdtHitChamber", &*m_mdtHitChamber);
  mytree->Branch ("mdtHitR", &*m_mdtHitR);
  mytree->Branch ("mdtHitZ", &*m_mdtHitZ);
  mytree->Branch ("mdtHitPhi", &*m_mdtHitPhi);
  mytree->Branch ("mdtHitresidual", &*m_mdtHitresidual);
  mytree->Branch ("mdtHitTime", &*m_mdtHitTime);
  mytree->Branch ("mdrHitSpace", &*m_mdrHitSpace);
  mytree->Branch ("mdtHitSigma", &*m_mdtHitSigma);
...
StatusCode MyxAODAnalysis :: execute ()
{
  ...
  //mdt
  m_nMdtHits->clear();
  m_mdtHitOnlineId->clear();
  m_mdtHitOfflineId->clear();
  m_mdtHitIsOutlier->clear();
  m_mdtHitChamber->clear();
  m_mdtHitR->clear();
  m_mdtHitZ->clear();
  m_mdtHitPhi->clear();
  m_mdtHitresidual->clear();
  m_mdtHitTime->clear();
  m_mdrHitSpace->clear();
  m_mdtHitSigma->clear();
  ...
  std::vector< TrigCompositeUtils::LinkInfo<xAOD::L2CombinedMuonContainer> > l2cbioLinks = m_trigDecisionTool->features<xAOD::L2CombinedMuonContainer>( "HLT_mu26_ivarmedium_mu6_l2io_probe_L1MU14FCH", TrigDefs::includeFailedDecisions, "HLT_MuonL2CBInfoIOmode" );
  for(const TrigCompositeUtils::LinkInfo<xAOD::L2CombinedMuonContainer>& l2cbioLinkInfo : l2cbioLinks){
    if( !l2cbioLinkInfo.isValid() ) continue;
    const ElementLink<xAOD::L2CombinedMuonContainer> l2cbio = l2cbioLinkInfo.link;
    if( !l2cbio.isValid() ) continue;

    bool pass = ( l2cbioLinkInfo.state == TrigCompositeUtils::ActiveState::ACTIVE );

    ANA_MSG_INFO ("execute(): " << " L2CBIO pt/eta/phi/pass = " << (*l2cbio)->pt() << "/" << (*l2cbio)->eta() << "/" << (*l2cbio)->phi() << "/" << pass);
  }
  ...
  for(size_t off_idx=0; off_idx<m_muon_eta->size(); off_idx++) {
    ...
    uint32_t nMdtHits = 0;
    std::vector<uint32_t > mdtHitOnlineId;
    std::vector<int > mdtHitOfflineId;
    ...
    std::vector<float > mdtHitTime;
    std::vector<float > mdrHitSpace;
    std::vector<float > mdtHitSigma;
    float dR = 0;
    float dRreq = 0.08;
    if(tlv_off.Pt() < 10)
    ...
    for(const TrigCompositeUtils::LinkInfo<xAOD::L2StandAloneMuonContainer>& l2saLinkInfo : l2saLinks){
      ...
      if(dR < dRmin) {
        ...
        */
        ...
        nMdtHits = (*l2sa)->nMdtHits();
        for(uint32_t i_tube = 0; i_tube < nMdtHits; i_tube++){
          mdtHitOnlineId.push_back((*l2sa)->mdtHitOnlineId(i_tube));
          mdtHitOfflineId.push_back((*l2sa)->mdtHitOfflineId(i_tube));
          mdtHitIsOutlier.push_back((*l2sa)->mdtHitIsOutlier(i_tube));
          mdtHitChamber.push_back((*l2sa)->mdtHitChamber(i_tube));
          mdtHitR.push_back((*l2sa)->mdtHitR(i_tube));
          mdtHitZ.push_back((*l2sa)->mdtHitZ(i_tube));
          mdtHitPhi.push_back((*l2sa)->mdtHitPhi(i_tube));
          mdtHitresidual.push_back((*l2sa)->mdtHitResidual(i_tube));
          mdtHitTime.push_back((*l2sa)->mdtHitTime(i_tube));
          mdrHitSpace.push_back((*l2sa)->mdtHitSpace(i_tube));
          mdtHitSigma.push_back((*l2sa)->mdtHitSigma(i_tube));
        }
      }
    ...
    m_nMdtHits->push_back(nMdtHits);

    m_mdtHitOnlineId->push_back(mdtHitOnlineId);
    m_mdtHitOfflineId->push_back(mdtHitOfflineId);
    m_mdtHitIsOutlier->push_back(mdtHitIsOutlier);
    m_mdtHitChamber->push_back(mdtHitChamber);
    m_mdtHitR->push_back(mdtHitR);
    m_mdtHitZ->push_back(mdtHitZ);
    m_mdtHitPhi->push_back(mdtHitPhi);
    m_mdtHitresidual->push_back(mdtHitresidual);
    m_mdtHitTime->push_back(mdtHitTime);
    m_mdrHitSpace->push_back(mdrHitSpace);
    m_mdtHitSigma->push_back(mdtHitSigma);
